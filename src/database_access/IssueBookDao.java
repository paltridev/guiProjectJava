...
public static boolean checkBook(String bookcallno){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("select * from books where SSN=?");
...
public static int save(String bookcallno,int studentid,String studentname,String studentcontact){
	...
	try{
		...
		if(status>0){
		PreparedStatement ps=con.prepareStatement("insert into issuebooks(bookSSN,studentid,studentname,studentcontact) values(?,?,?,?)");
		ps.setString(1,bookcallno);
